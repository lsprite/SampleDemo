...
public class AdvancedWebView extends WebView {
	...
	public interface Listener {
		...
		void onDownloadRequested(String url, String suggestedFilename,
	...
	/**
	 * Alternative browsers that have their own rendering engine and *may* be
	...
	protected static final String[] ALTERNATIVE_BROWSERS = new String[] {
			...
			"org.mozilla.firefox_beta", "com.chrome.beta",
	 ...
	 *            the URL to use as the page's base URL
	 * @param historyUrl
	 *            the URL to use for the page's history entry
	 */
	public void loadHtml(final String html, final String baseUrl,
	 ...
	 *            the URL to use for the page's history entry
	 * @param encoding
	 *            the encoding or charset of the HTML source text
	 */
	public void loadHtml(final String html, final String baseUrl,
					...
					if (mFileUploadCallbackFirst != null) {
						...
						try {
							if (intent.getDataString() != null) {
								dataUris = new Uri[] { Uri.parse(intent
										.getDataString()) };
							} else {
								if (Build.VERSION.SDK_INT >= 16) {
									if (intent.getClipData() != null) {
										final int numSelectedFiles = intent
												.getClipData().getItemCount();

										dataUris = new Uri[numSelectedFiles];

										for (int i = 0; i < numSelectedFiles; i++) {
											dataUris[i] = intent.getClipData()
													.getItemAt(i).getUri();
										}
									}
								}
	...
	/**
	 * Adds an additional HTTP header that will be sent along with every HTTP
	 * `GET` request
	 * 
	 * This does only affect the main requests, not the requests to included
	 * resources (e.g. images)
	 * 
	 * If you later want to delete an HTTP header that was previously added this
	 * way, call `removeHttpHeader()`
	 * 
	 * The `WebView` implementation may in some cases overwrite headers that you
	 * set or unset
	 * 
	 * @param name
	 *            the name of the HTTP header to add
	 * @param value
	 *            the value of the HTTP header to send
	 */
	...
	/**
	 * Removes one of the HTTP headers that have previously been added via
	 * `addHttpHeader()`
	 * 
	 * If you want to unset a pre-defined header, set it to an empty string with
	 * `addHttpHeader()` instead
	 * 
	 * The `WebView` implementation may in some cases overwrite headers that you
	 * set or unset
	 * 
	...
	@SuppressLint("NewApi")
	protected static void setAllowAccessFromFileUrls(
			final WebSettings webSettings, final boolean allowed) {
		if (Build.VERSION.SDK_INT >= 16) {
	...
	@SuppressWarnings("static-method")
	@SuppressLint("NewApi")
	protected void setMixedContentAllowed(final WebSettings webSettings,
			final boolean allowed) {
		if (Build.VERSION.SDK_INT >= 21) {
			webSettings
					.setMixedContentMode(allowed ? WebSettings.MIXED_CONTENT_ALWAYS_ALLOW
	...
	public void setDesktopMode(final boolean enabled) {
		...
		if (enabled) {
			newUserAgent = webSettings.getUserAgentString()
					.replace("Mobile", "eliboM").replace("Android", "diordnA");
		} else {
			newUserAgent = webSettings.getUserAgentString()
					.replace("eliboM", "Mobile").replace("diordnA", "Android");
		}
	...
	protected void init(Context context) {
		...
		final String databaseDir = filesDir.substring(0,
		...
		super.setWebViewClient(new WebViewClient() {
			...
			public void onReceivedError(WebView view, int errorCode,
				...
				if (mCustomWebViewClient != null) {
					mCustomWebViewClient.onReceivedError(view, errorCode,
			...
			public boolean shouldOverrideUrlLoading(final WebView view,
				...
				if (mCustomWebViewClient != null) {
					// if the user-specified handler asks to override the
					// request
			...
			public WebResourceResponse shouldInterceptRequest(WebView view,
				...
				if (Build.VERSION.SDK_INT >= 11) {
					if (mCustomWebViewClient != null) {
						return mCustomWebViewClient.shouldInterceptRequest(
								view, url);
					} else {
			...
			public void onFormResubmission(WebView view, Message dontResend,
					Message resend) {
				if (mCustomWebViewClient != null) {
					mCustomWebViewClient.onFormResubmission(view, dontResend,
							resend);
				} else {
			...
			public void doUpdateVisitedHistory(WebView view, String url,
					boolean isReload) {
				if (mCustomWebViewClient != null) {
					mCustomWebViewClient.doUpdateVisitedHistory(view, url,
							isReload);
				} else {
			...
			public void onReceivedSslError(WebView view,
				...
				if (mCustomWebViewClient != null) {
					mCustomWebViewClient.onReceivedSslError(view, handler,
							error);
				} else {
			...
			public void onReceivedClientCertRequest(WebView view,
				...
				if (Build.VERSION.SDK_INT >= 21) {
					if (mCustomWebViewClient != null) {
						mCustomWebViewClient.onReceivedClientCertRequest(view,
								request);
					} else {
			...
			public void onReceivedHttpAuthRequest(WebView view,
				...
				if (mCustomWebViewClient != null) {
					mCustomWebViewClient.onReceivedHttpAuthRequest(view,
							handler, host, realm);
				} else {
			...
			public boolean shouldOverrideKeyEvent(WebView view, KeyEvent event) {
				if (mCustomWebViewClient != null) {
					return mCustomWebViewClient.shouldOverrideKeyEvent(view,
							event);
				} else {
			...
			public void onScaleChanged(WebView view, float oldScale,
					float newScale) {
				if (mCustomWebViewClient != null) {
					mCustomWebViewClient.onScaleChanged(view, oldScale,
							newScale);
				} else {
			...
			public void onReceivedLoginRequest(WebView view, String realm,
				...
				if (Build.VERSION.SDK_INT >= 12) {
					if (mCustomWebViewClient != null) {
						mCustomWebViewClient.onReceivedLoginRequest(view,
								realm, account, args);
					} else {
		...
		super.setWebChromeClient(new WebChromeClient() {
			...
			public void onReceivedTouchIconUrl(WebView view, String url,
					boolean precomposed) {
				if (mCustomWebChromeClient != null) {
					mCustomWebChromeClient.onReceivedTouchIconUrl(view, url,
							precomposed);
				} else {
			...
			public void onShowCustomView(View view, int requestedOrientation,
				...
				if (Build.VERSION.SDK_INT >= 14) {
					if (mCustomWebChromeClient != null) {
						mCustomWebChromeClient.onShowCustomView(view,
			...
			public boolean onCreateWindow(WebView view, boolean isDialog,
				...
				if (mCustomWebChromeClient != null) {
					return mCustomWebChromeClient.onCreateWindow(view,
			...
			public boolean onJsAlert(WebView view, String url, String message,
					JsResult result) {
				if (mCustomWebChromeClient != null) {
					return mCustomWebChromeClient.onJsAlert(view, url, message,
							result);
				} else {
			...
			public boolean onJsConfirm(WebView view, String url,
				...
				if (mCustomWebChromeClient != null) {
					return mCustomWebChromeClient.onJsConfirm(view, url,
							message, result);
				} else {
			...
			public boolean onJsPrompt(WebView view, String url, String message,
				...
				if (mCustomWebChromeClient != null) {
					return mCustomWebChromeClient.onJsPrompt(view, url,
			...
			public boolean onJsBeforeUnload(WebView view, String url,
					String message, JsResult result) {
				if (mCustomWebChromeClient != null) {
					return mCustomWebChromeClient.onJsBeforeUnload(view, url,
							message, result);
				} else {
			...
			public void onConsoleMessage(String message, int lineNumber,
					String sourceID) {
				if (mCustomWebChromeClient != null) {
					mCustomWebChromeClient.onConsoleMessage(message,
							lineNumber, sourceID);
				} else {
			...
			public void onExceededDatabaseQuota(String url,
				...
				if (mCustomWebChromeClient != null) {
					mCustomWebChromeClient.onExceededDatabaseQuota(url,
					...
					super.onExceededDatabaseQuota(url, databaseIdentifier,
							quota, estimatedDatabaseSize, totalQuota,
							quotaUpdater);
				}
			}
			...
			public void onReachedMaxAppCacheSize(long requiredStorage,
				...
				if (mCustomWebChromeClient != null) {
					mCustomWebChromeClient.onReachedMaxAppCacheSize(
							requiredStorage, quota, quotaUpdater);
				} else {
		...
		setDownloadListener(new DownloadListener() {
			...
			public void onDownloadStart(final String url,
				...
				final String suggestedFilename = URLUtil.guessFileName(url,
						contentDisposition, mimeType);

				if (mListener != null) {
					mListener.onDownloadRequested(url, suggestedFilename,
							mimeType, contentLength, contentDisposition,
							userAgent);
				}
			}
	...
	protected boolean isHostnameAllowed(final String url) {
		...
		for (String expectedHost : mPermittedHostnames) {
			// if the two hostnames match or if the actual host is a subdomain
			// of the expected host
		...
		// the actual hostname of the URL to be checked is not allowed since
	...
	/**
	 * Provides localizations for the 25 most widely spoken languages that have
	 * a ISO 639-2/T code
	 * 
	...
	protected void openFileInput(
			final ValueCallback<Uri> fileUploadCallbackFirst,
	...
	/**
	 * Returns whether file uploads can be used on the current device (generally
	 * all platform versions except for 4.4)
	 * 
	 ...
	 * all platform versions except for 4.4)
	 * 
	 * On Android 4.4.3/4.4.4, file uploads may be possible but will come with a
	 * wrong MIME type
	 * 
	 * @param needsCorrectMimeType
	 *            whether a correct MIME type is required for file uploads or
	 *            `application/octet-stream` is acceptable
	 * @return whether file uploads can be used
	 */
	public static boolean isFileUploadAvailable(
			final boolean needsCorrectMimeType) {
		if (Build.VERSION.SDK_INT == 19) {
			final String platformVersion = (Build.VERSION.RELEASE == null) ? ""
					: Build.VERSION.RELEASE;

			return !needsCorrectMimeType
					&& (platformVersion.startsWith("4.4.3") || platformVersion
							.startsWith("4.4.4"));
		} else {
			return true;
		}
	...
	/**
	 * Handles a download by loading the file from `fromUrl` and saving it to
	 * `toFilename` on the external storage
	 * 
	 ...
	 * @param fromUrl
	 *            the URL of the file to download, e.g. the one from
	 *            `AdvancedWebView.onDownloadRequested(...)`
	 * @param toFilename
	 *            the name of the destination file where the download should be
	 *            saved, e.g. `myImage.jpg`
	 * @return whether the download has been successfully handled or not
	 * @throws IllegalStateException
	 *             if the storage or the target directory could not be found or
	 *             accessed
	 */
	@SuppressLint("NewApi")
	public static boolean handleDownload(final Context context,
			final String fromUrl, final String toFilename) {
		...
		request.setDestinationInExternalPublicDir(
				Environment.DIRECTORY_DOWNLOADS, toFilename);
		...
		// if the download manager app has been disabled on the device
			...
			// show the settings screen where the user can enable the download
			// manager app again
	...
	/**
	 * Wrapper for methods related to alternative browsers that have their own
	 * rendering engines
	 */
	public static class Browsers {

		/**
		 * Package name of an alternative browser that is installed on this
		 * device
		 */
		private static String mAlternativePackage;

		/**
		 * Returns whether there is an alternative browser with its own
		 * rendering engine currently installed
		 * 
		 * @param context
		 *            a valid `Context` reference
		...
		/**
		 * Returns the package name of an alternative browser with its own
		 * rendering engine or `null`
		 * 
		 * @param context
		 *            a valid `Context` reference
		...
		public static String getAlternative(final Context context) {
			...
			final List<String> alternativeBrowsers = Arrays
					.asList(ALTERNATIVE_BROWSERS);
		 ...
		 * @param url
		 *            the URL to open
		 * @param withoutTransition
		 *            whether to switch to the browser `Activity` without a
		 *            transition
		 */
		public static void openUrl(final Activity context, final String url,
