...
public class SMS4 {
	...
	public static byte[] encodeSMS4(String plaintext) {
		...
		return SMS4.encodeSMS4(plaintext.getBytes());
	}
	 ...
	 * @return
	 */
	public static byte[] encodeSMS4(byte[] plaintext) {
		...
		while (k + 16 <= plainLen) {
			...
			byte[] cellCipher = encode16(cellPlain);
	 ...
	 * @return
	 */
	public static byte[] decodeSMS4(byte[] ciphertext) {
		...
		while (k + 16 <= cipherLen) {
			...
			byte[] cellPlain = decode16(cellCipher);
	 ...
	 * @return
	 */
	public static String decodeSMS4toString(byte[] ciphertext) {
		byte[] plaintext = new byte[ciphertext.length];
		plaintext = decodeSMS4(ciphertext);
	 ...
	 * @return
	 */
	public static String decodeSMS4toString(String ciphertext) {
		...
		try {
			cipherbyte = Hex.decodeHex(cipherchar);
		} catch (DecoderException e) {
			e.printStackTrace();
		}
		byte[] plaintext = new byte[cipherbyte.length];
		plaintext = decodeSMS4(cipherbyte);
		return new String(plaintext).trim();
	}
	 ...
	 * @return
	 */
	private static byte[] encode16(byte[] plaintext) {
	 ...
	 * @return
	 */
	private static byte[] decode16(byte[] ciphertext) {
	 ...
	 * @return
	 */
	private static byte[] encode32(byte[] plaintext) {
	 ...
	 * @return
	 */
	private static byte[] decode32(byte[] plaintext) {
		byte[] plain = new byte[32];
		SMS4 sm4 = new SMS4();
		sm4.sms4(plaintext, 32, KEY, plain, DECRYPT);

		return plain;
	}

	public static void test() {
		// String newString = "111111送达倒萨多萨达sadadasdsadasdsadsdsadsadsd"; // 明文
		String newString = "/n \n1111sadas达!@#$%^&*()_+-=！@#￥%……&*（）——+倒萨多{}{}:\"\"das达倒萨多"; // 明文
		System.out.println("原文:" + newString);
		// String endodeStr = new
		// String(Hex.encodeHex(SMS4.encodeSMS4(newString)));
		String endodeStr = encodeSMS4toString(newString);
		System.out.println("加密后:" + endodeStr);
		String decodeStr = decodeSMS4toString(endodeStr);
		System.out.println("解密后:" + decodeStr);
	}

	public static String encodeSMS4toString(String ciphertext) {
		if (TextUtils.isEmpty(ciphertext)) {
			return "";
		}
		return new String(Hex.encodeHex(SMS4.encodeSMS4(ciphertext)));
	}
}
