...
public class RootShell {
	...
	/**
	 * Use this to check whether or not a file OR directory exists on the
	 * filesystem.
	 * 
	 * @param file
	 *            String that represent the file OR the directory, including the
	 *            full path to the file and its name.
	 * @param isDir
	 *            boolean that represent whether or not we are looking for a
	 *            directory
	 * @return a boolean that will indicate whether or not the file exists.
	 */
	 ...
	 * @param singlePath
	 *            boolean that represents whether to return a single path or
	 *            multiple.
	 * 
	 * @return <code>List<String></code> containing the locations the binary was
	 *         found at.
	 */
	...
	/**
	 * @param binaryName
	 *            <code>String</code> that represent the binary to find.
	 * @param searchPaths
	 *            <code>List<String></code> which contains the paths to search
	 *            for this binary in.
	 * @param singlePath
	 *            boolean that represents whether to return a single path or
	 *            multiple.
	...
	public static List<String> findBinary(final String binaryName,
			List<String> searchPaths, boolean singlePath) {
				...
				Command cc = new Command(0, false, "stat " + path + binaryName) {
					...
					public void commandOutput(int id, String line) {
						if (line.contains("File: ")
							...
							RootShell.log(binaryName + " was found here: "
									+ currentPath);
						}
	 ...
	 * @param shellContext
	 *            the context to execute the shell with
	 * @param retry
	...
	public static Shell getShell(boolean root, int timeout,
			Shell.ShellContext shellContext, int retry) throws IOException,
	 ...
	 */
	public static Shell getShell(boolean root, int timeout,
			Shell.ShellContext shellContext) throws IOException,
			TimeoutException, RootDeniedException {
	...
	public static boolean isAccessGiven(final int timeout, final int retries) {
		final Set<String> ID = new HashSet<String>();
		final int IAG = 158;
		try {
			log("Checking for Root access");
			final Command command = new Command(158, false,
					new String[] { "id" }) {
				@Override
			...
			final Shell shell = Shell.startRootShell(timeout, retries);
			shell.add(command);
	 ...
	 * depending whether or not RootShell.debugMode is on. So you can use this
	 ...
	 *            The message to output.
	 * @param type
	 *            The type of log, 1 for verbose, 2 for error, 3 for debug, 4
	 *            for warn
	 * @param e
	...
	private static void commandWait(Shell shell, Command cmd) throws Exception {
		while (!cmd.isFinished()) {
			...
			RootShell.log(version, "Processed " + cmd.totalOutputProcessed
					+ " of " + cmd.totalOutput + " output from command.");
